...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const baselineConstraints = await loadConstraints(db, accountId, clientId)
    ...
    // overrides applied.
    const baselineSnapshots = await loadLatestModuleSnapshots(db, accountId, clientId)
    ...
    const scenarioResults = await runAllModules(db, accountId, clientId, {
      baselineConstraints,
