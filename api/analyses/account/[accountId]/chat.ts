...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'DELETE') {
    await db.from('analysis_chat_messages').delete().eq('account_id', accountId)
  ...
  const properties = await loadAccountProperties(db, accountId, constraints.client_id)
  ...
  try {
    switch (name) {
      case 'get_module_output': {
        const snaps = await loadLatestModuleSnapshots(c.db, c.accountId)
      ...
      case 'simulate_scenario': {
        ...
        const baseline = await loadConstraints(c.db, c.accountId)
        const results = await runAllModules(c.db, c.accountId, {
            ...
            baseline_summary: {
              bid_total:
                baseline.selected_branches
                  ? (await loadLatestModuleSnapshots(c.db, c.accountId))
